...
// === CREAR USUARIO, EDITAR, PERMISOS POR ROL ===
            ...
            <div id="contenido_roles" class="hidden">
                <div class="bg-gradient-to-r from-blue-900 to-blue-700 rounded-xl p-8 text-white">
                    ...
                    <form method="POST" class="space-y-6">
                      ...
                      <!-- CONFIGURACIÓN DE EMAIL - SIN DUPLICADOS AL REFRESCAR -->
            <div id="contenido_email" class="hidden">
                <?php
                // === GUARDAR CONFIGURACIÓN CON REDIRECCIÓN ===
                if(isset($_POST['guardar_email'])){
                    $datos = [
                    ...
                    try {
                        $sql = "INSERT INTO config_email 
                                (smtp_host, smtp_port, smtp_user, smtp_pass, smtp_from_email, smtp_from_name, smtp_secure, notificar_usuarios) 
                                VALUES (?, ?, ?, ?, ?, ?, ?, ?) 
                                ...
                                notificar_usuarios = VALUES(notificar_usuarios)";

                        $stmt = $pdo->prepare($sql);
                        $stmt->execute(array_values($datos));
                        
                        // REDIRECCIÓN PARA EVITAR DUPLICADOS
                        header("Location: mantenimiento.php?tab=email&msg=guardado");
                        exit();
                    } catch(Exception $e){
                        $msg = "<span class='text-red-600 font-bold'>ERROR SQL: " . htmlspecialchars($e->getMessage()) . "</span>";
                    }
                }

                // === PRUEBA DE ENVÍO CON REDIRECCIÓN ===
                if(isset($_POST['probar_email'])){
                    $test_email = trim($_POST['test_email'] ?? '');
                    if(empty($test_email) || !filter_var($test_email, FILTER_VALIDATE_EMAIL)){
                        header("Location: mantenimiento.php?tab=email&prueba=error_correo");
                        exit();
                    } else {
                        ...
                        try {
                            ...
                            $mail->Body    = "<h2>PRUEBA EXITOSA</h2><p>Funciona con cualquier proveedor.</p>";

                            $mail->send();
                            header("Location: mantenimiento.php?tab=email&prueba=enviado");
                            exit();
                        } catch (Exception $e) {
                            header("Location: mantenimiento.php?tab=email&prueba=error&detalle=" . urlencode($mail->ErrorInfo));
                            exit();
                        }
                    }
                }
                ...
                if(isset($_GET['msg']) && $_GET['msg'] === 'guardado'){
                    $msg = "<span class='text-green-600 font-bold text-2xl'>CONFIGURACIÓN GUARDADA CORRECTAMENTE</span>";
                }
                if(isset($_GET['prueba'])){
                    if($_GET['prueba'] === 'enviado'){
                        $prueba_msg = "<span class='text-green-600 font-bold text-2xl'>PRUEBA ENVIADA CORRECTAMENTE</span>";
                    } elseif($_GET['prueba'] === 'error_correo'){
                        $prueba_msg = "<span class='text-red-600 font-bold'>Ingresa un correo válido</span>";
                    } elseif($_GET['prueba'] === 'error'){
                        $prueba_msg = "<span class='text-red-600 font-bold'>ERROR: " . htmlspecialchars($_GET['detalle'] ?? 'Desconocido') . "</span>";
                    }
                }
                ?>
                <div class="bg-gradient-to-r from-blue-900 to-cyan-700 rounded-2xl shadow-2xl p-10 text-white">
                    <h2 class="text-3xl font-bold mb-8 text-center">
                        CONFIGURACIÓN UNIVERSAL DE CORREO
                    </h2>

                    <?php if(isset($msg)): ?>
                    <div class="bg-white bg-opacity-20 border-4 border-green-400 text-green-100 px-8 py-6 rounded-2xl mb-10 text-xl font-bold text-center">
                        <?= $msg ?>
                    </div>
                    <?php endif; ?>
                    ...
                    <form method="POST" class="grid grid-cols-1 md:grid-cols-2 gap-8">
                        ...
                        <div>
                            ...
                            <input type="text" name="smtp_host" value="<?= htmlspecialchars($config_email['smtp_host'] ?? '') ?>" required 
                                   class="w-full p-4 rounded-lg text-black text-lg">
                        </div>
                        <div>
                            <label class="block text-lg font-bold mb-3">Puerto</label>
                            <input type="number" name="smtp_port" value="<?= $config_email['smtp_port'] ?? 587 ?>" required 
                                   class="w-full p-4 rounded-lg text-black text-lg">
                        </div>
                        <div>
                            <label class="block text-lg font-bold mb-3">Modo de Cifrado</label>
                            <select name="smtp_secure" class="w-full p-4 rounded-lg text-black text-lg">
                                <option value="" <?= ($config_email['smtp_secure'] ?? '')==''?'selected':'' ?>>Ninguno</option>
                                <option value="tls" <?= ($config_email['smtp_secure'] ?? '')=='tls'?'selected':'' ?>>TLS</option>
                                <option value="ssl" <?= ($config_email['smtp_secure'] ?? '')=='ssl'?'selected':'' ?>>SSL</option>
                            </select>
                        </div>
                        <div>
                        ...
                        <div>
                            <label class="block text-lg font-bold mb-3">Contraseña</label>
                        ...
                        <div>
                            ...
                            <input type="text" name="smtp_from_name" value="<?= htmlspecialchars($config_email['smtp_from_name'] ?? 'Sansouci Desk') ?>" required 
                                   class="w-full p-4 rounded-lg text-black text-lg">
                        </div>
                        <div>
                            <label class="block text-lg font-bold mb-3">Correo para Prueba</label>
                            <input type="email" name="test_email" value="<?= htmlspecialchars($config_email['smtp_user'] ?? '') ?>" placeholder="prueba@tuemail.com" 
                                   class="w-full p-4 rounded-lg text-black text-lg">
                        </div>
                        <div class="md:col-span-2">
                            <label class="flex items-center space-x-4 cursor-pointer">
                                <input type="checkbox" name="notificar_usuarios" <?= ($config_email['notificar_usuarios'] ?? 1)?'checked':'' ?> 
                                       class="w-8 h-8 text-yellow-400">
                                <span class="text-xl font-bold">Notificar a todos los agentes</span>
                            </label>
                        </div>
                        <div class="md:col-span-2 text-center space-x-8">
                            <button type="submit" name="probar_email" 
                                    class="bg-orange-500 text-white px-20 py-8 rounded-full text-2xl font-bold hover:bg-orange-600">
                                PROBAR ENVÍO
                            </button>
                            <button type="submit" name="guardar_email" 
                                    class="bg-yellow-400 text-blue-900 px-32 py-12 rounded-full text-4xl font-bold hover:bg-yellow-300">
