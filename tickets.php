...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['seleccionar'])) {
    ...
    if (empty($seleccionados)) {
        ...
        if ($accion === 'eliminar' && ($user['rol'] ?? '') === 'superadmin') {
            $eliminados = TicketModel::deleteTicketsAndResponses($seleccionados);
            $mensaje    = $eliminados . " ticket(s) eliminados";
        }
...
// $user viene de header.php (sesión)
$tickets = TicketModel::getTicketsForUser($user, $estadoFiltro);
?>

<h1 class="text-4xl font-bold text-blue-900 mb-10">
    Gestión de Tickets (<?= count($tickets) ?>)
</h1>
...
<div class="bg-white rounded-2xl shadow-xl p-8 mb-10 flex flex-wrap items-center justify-between gap-6">
    <div class="flex flex-wrap gap-6">
        <a href="tickets.php"
           class="px-8 py-4 bg-gray-200 rounded-xl font-bold text-xl <?= !isset($_GET['estado']) ? 'bg-blue-900 text-white' : '' ?>">
            Todos
        </a>
        <a href="tickets.php?estado=abierto"
           class="px-8 py-4 bg-green-200 rounded-xl font-bold text-xl <?= ($_GET['estado'] ?? '')=='abierto' ? 'bg-green-700 text-white' : '' ?>">
            Abiertos
        </a>
        <a href="tickets.php?estado=en_proceso"
           class="px-8 py-4 bg-yellow-200 rounded-xl font-bold text-xl <?= ($_GET['estado'] ?? '')=='en_proceso' ? 'bg-yellow-700 text-white' : '' ?>">
            En Proceso
        </a>
        <a href="tickets.php?estado=cerrado"
           class="px-8 py-4 bg-gray-500 text-white rounded-xl font-bold text-xl <?= ($_GET['estado'] ?? '')=='cerrado' ? 'bg-gray-700' : '' ?>">
            Cerrados
        </a>
    </div>
        ...
        <table class="w-full table-auto">
            ...
            <tbody>
                ...
                <tr class="border-b-4 border-blue-100 hover:bg-blue-50 transition text-lg">
                    ...
                    <td class="p-6">
                        <span class="px-6 py-3 rounded-full font-bold <?= $t['estado']=='abierto'
                    ...
                    <td class="p-6">
                        <span class="px-6 py-3 rounded-full font-bold <?= $t['prioridad']=='urgente'
